...
if (empty($_SESSION['cart'])) {
    echo "<center><p>ตะกร้าสินค้าของคุณว่างเปล่า</p></center>";
    exit;
}

// แปลง id ของสินค้าให้เป็น integer เพื่อความถูกต้อง
$itemIds = array_keys($_SESSION['cart']); // ดึง keys ของตะกร้า
$itemIds = array_map('intval', $itemIds); // แปลงทุก id เป็น integer
$itemIds = implode(',', $itemIds); // แปลงเป็น string ที่แยกด้วย comma
...
$sql = "SELECT * FROM product WHERE Iditem IN ($itemIds)";
...
$shipping_method = trim($_POST['shipping-method']);

// ตรวจสอบข้อมูลการจัดส่ง
if (empty($name) || empty($address) || empty($phone) || empty($shipping_method)) {
    echo "<script>alert('กรุณากรอกข้อมูลทั้งหมด'); window.history.back();</script>";
    exit;
}

// บันทึกข้อมูลคำสั่งซื้อในฐานข้อมูล
$sql_order = "INSERT INTO orders (Username, CustomerName, Address, Phone, ShippingMethod, TotalAmount) 
              VALUES (?, ?, ?, ?, ?, ?)";
...
$stmt->bind_param("sssssd", $username, $name, $address, $phone, $shipping_method, $totalAmount);
$stmt->execute();
$orderID = $stmt->insert_id; // ดึง ID คำสั่งซื้อที่เพิ่งเพิ่มเข้าไป
$stmt->close();

// บันทึกสินค้าที่สั่งซื้อ
$sql_item = "INSERT INTO order_items (OrderID, ProductID, Quantity, Price) VALUES (?, ?, ?, ?)";
...
// ล้างตะกร้าสินค้าหลังจากทำการสั่งซื้อ
unset($_SESSION['cart']);
?>

<?php
// ดึงข้อมูลคำสั่งซื้อจากฐานข้อมูล
$orderID = $orderID; // หรือดึงจากตัวแปร session ถ้ามี
$sql_order_items = "SELECT oi.Quantity, oi.Price, p.Name FROM order_items oi
                    JOIN product p ON oi.ProductID = p.Iditem
                    WHERE oi.OrderID = ?";
